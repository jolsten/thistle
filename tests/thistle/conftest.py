...
ISS_TLES = read_tle("tests/thistle/data/25544.tle")
ISS_SATRECS = [Satrec.twoline2rv(a, b) for a, b in ISS_TLES]

DAILY_FILES = list(pathlib.Path("tests/thistle/data/day").glob("*.txt"))
OBJECT_FILES = list(pathlib.Path("tests/thistle/data/obj").glob("*.txt"))
